...
| 版本 | 日期 | 变更摘要 |
| --- | --- | --- |
| v1.14 | 2024-06-17 | 更新 `nop-sys-dao` 证据矩阵，新增“消息处理失败”行并规划采集 `nop.err.sys.process-event-fail`/`nop.err.sys.handle-process-event-error-fail` 与事件字段对照日志，路线图同步加入行动 #35。 |
...
| v1.05 | 2024-06-17 | 补充 `nop-sys-dao` 文档的消息线程回退 Run-less 计划与证据矩阵引用，强调采集 `GlobalExecutors` 回退日志、覆盖 `executor`/`timer` 时同步配置，路线图提醒将默认线程采样纳入待办。 |
...
| v0.96 | 2024-06-17 | 更新 `nop-sys-dao` 文档，补充序列缓存需要等待 `defaultCache` TTL 或重启才会生效的运维规则、新的 Run-less 用例与证据矩阵行，并要求记录清缓存前后返回值以验证独立事务与缓存占位影响；路线图同步强调序列缓存刷新取证。 |
